...
const taskListCompleted = $("#completed-tasks");
const taskListIncomplete = $("#incomplete-tasks");
const todoInput = $("#todo-input");
const submit = $("#submit");
const form = $("#todo-form");
const taskMenu = $("#task-menu");
const filterCompleted = $("#filter-done");
const filterIncomplete = $("#filter-not-done");
...
function isDuplication(title, index = -1) {
    return tasks.some(
        (task, i) => i !== index && task.title.toLowerCase() === title.toLowerCase()
    );
}
...
taskListCompleted.addEventListener("click", handleTaskAction);
taskListIncomplete.addEventListener("click", handleTaskAction);
...
function switchView(view) {
    ...
    taskListCompleted.classList.add("none");
    taskListIncomplete.classList.add("none");

    if (view === "done") {
        render(filterTasks(true), taskListCompleted);
        taskListCompleted.classList.remove("none");
        return;
    }
    if (view === "not-done") {
        render(filterTasks(false), taskListIncomplete);
        taskListIncomplete.classList.remove("none");
        return;
    }
    render(tasks, taskList);
